...
class HowLongToBeat(scrapy.Spider):
    ...
    def parse_game_data(self, response):
        ...
        raw_genre = response.xpath('//div[contains(@class, "profile_info")]/strong[contains(text(), "Genre")]/following-sibling::text()[2]').get()
        raw_genres = response.xpath(
            '//div[contains(@class, "profile_info")]/strong[contains(text(), "Genres")]/following-sibling::text()[2]').get()
        if raw_genre:
            genre = [x.strip() for x in raw_genre.split(",")]
            item['genres'] = genre
        elif raw_genres:
        ...
        main_story_raw = response.xpath('//li[contains(@class, "short")]/*[contains(text(), "Main Story")]/following-sibling::div/text()').get()
        item['main_story'] = fraction_normalisation(main_story_raw)
        main_plus_extras_raw = response.xpath('//li[contains(@class, "short")]/*[contains(text(), "Main + Extras")]/following-sibling::div/text()').get()
